...
EventBus.on('EmailFailed', async (payload) => {
  try {
    ...
    const rawEventType = eventData?.event || eventData?.fetchedEventType || 'failed';
    const failureReason = eventData?.reason || `Terminal failure: ${rawEventType}`;
    ...
    // Get max retries and check if should mark as dead
    ...
    console.log(`[FailedHandler] Paused ${pausedJobs.count} pending jobs for lead ${job.leadId}`);
    ...
    if (shouldMarkDead) {
      ...
      await StatusUpdateService._recalculateStatus(job.leadId, eventType, job.type);
    ...
    console.log(`[FailedHandler] Processed ${eventType} for ${lead.email} - Manual retry required`);
